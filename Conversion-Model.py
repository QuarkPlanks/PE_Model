...
def data_process(x):
    df = pd.DataFrame(x)
    df[5] = 3.97e-5*x[:,2]*x[:,3]*x[:,4]*np.exp(3385*(1/(x[:,1]+273.15)))/(1+3.97e-5*x[:,2]*x[:,3]*x[:,4]*np.exp(3385*(1/(x[:,1]+273.15))))
    return np.array(df)

# Import and process data
from sklearn.preprocessing import StandardScaler

train_x, train_y_con, train_y_BD = read_data(r'data/train_con.xlsx')
train_x = data_process(train_x)
test_x, test_y_con, test_y_BD = read_data(r'data/test_con.xlsx')
...
for model, name in zip(models, model_names):
    ...
    trained_model, mse = model_train(train_x_scaled, train_y_con, model)
...
for name, model in trained_models.items():
    ...
    predictions, metrics = model_validate(test_x_scaled, test_y_con, model)
    plot_predictions(test_y_con, predictions, title=f'{name} Model Predictions')
...
# Save Model
import os
train_x, train_y_con, train_y_BD = read_data(r'data/conversion.xlsx')
train_x = data_process(train_x)
train_x_scaled = scaler.fit_transform(train_x)
model_names = ['Linear']
models = [Linear]
...
for model, name in zip(models, model_names):
    ...
    trained_models[name] = trained_model
    print(f"{name} model trained with MSE: {mse:.4f}")
save_model(trained_models['Linear'], 'conversion.pkl')
save_model(scaler, 'scaler_con.pkl')
